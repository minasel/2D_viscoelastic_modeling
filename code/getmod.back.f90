...
!        do ix=-lv,nx+lv
!                do iz=-lv,nz+lv
!                vs(iz,ix)=1000*vs(iz,ix)
!                vp(iz,ix)=1000*vp(iz,ix)
!                rho(iz,ix)=1000*rho(iz,ix)
!                enddo
!        enddo
      ! read in tau modules
            do m=1, nq
!
               call ficnum(m, cm)
               filename = datapath(idpmin:idpmax)//"smfai1"//cm
...
!
!
               filename = datapath(idpmin:idpmax)//"smfai2"//cm
               call centra(filename, imin, imax)
               open(88, file=filename(imin:imax), access='direct',&
                   form='unformatted', recl=mac_len*nz)
               do ix=1, nx
